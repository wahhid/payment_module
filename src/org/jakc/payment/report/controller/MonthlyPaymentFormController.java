...
import org.jakc.payment.report.gui.MonthlyPaymentFormFrame;
 ...
 * @author wahhid
 */
public class MonthlyPaymentFormController {
    
    private ProcessCallBack pcb;
    private BackingController bc;    
    private Connection conn;
    private Auth auth;
    private MonthlyPaymentFormFrame frame;    
    private String filePath;   
    private InputStream streamReport;
    
    public MonthlyPaymentFormController(Connection conn, Auth auth){
        this.conn = conn;
        this.auth = auth;
        this.bc = new BackingController(this.conn);        
        this.frame = new MonthlyPaymentFormFrame(this);        
        MDIFrame.add(this.frame);                
    }                       
   
    public ProcessCallBack PrepareDailyReport(Date starttanggal , Date endtanggal, int billingstatus, int approvedstatus){
        List<RequestTransaksiStikerRpt> orpts = new ArrayList();        
        
        this.pcb = this.bc.getRequestTransaksiStikerController().ForDailyReport(starttanggal,endtanggal,billingstatus,approvedstatus);        
        if(this.pcb.isError()){
            return this.pcb;
        }
